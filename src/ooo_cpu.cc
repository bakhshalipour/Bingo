...
void O3_CPU::add_load_queue(uint32_t rob_index, uint32_t data_index)
{
    ...
    if (LQ.entry[lq_index].virtual_address && (LQ.entry[lq_index].producer_id == UINT64_MAX)) { // not released and no forwarding
        ...
        cout << " head: " << RTL0_head << " tail: " << RTL0_tail << "\tcpu = " << cpu << endl; }); 
    }
    ...
    cout << " fetched: " << +LQ.entry[lq_index].fetched << " index: " << lq_index << " occupancy: " << LQ.occupancy << " cycle: " << current_core_cycle[cpu] << "\tcpu = " << cpu << endl; });
}
...
void O3_CPU::mem_RAW_dependency(uint32_t prior, uint32_t current, uint32_t data_index, uint32_t lq_index)
{
    for (uint32_t i=0; i<MAX_INSTR_DESTINATIONS; i++) {
        ...
        if (ROB.entry[prior].destination_memory[i] == ROB.entry[current].source_memory[data_index]) { //  store-to-load forwarding check
            ...
            cout << hex << " address: " << ROB.entry[prior].destination_memory[i] << dec << "\tcpu = " << cpu << endl; });
...
void O3_CPU::add_store_queue(uint32_t rob_index, uint32_t data_index)
{
    ...
    */
    ...
    cout << " cycle: " << current_core_cycle[cpu] << "\tcpu = " << cpu << endl; });
}
    ...
    while (store_issued < SQ_WIDTH) {
        if (RTS0[RTS0_head] < SQ_SIZE) {
            ...
            if (SQ.entry[sq_index].event_cycle <= current_core_cycle[cpu]) {
                ...
                cout << " head: " << RTS0_head << " tail: " << RTS0_tail << "\tcpu = " << cpu << endl; }); 
...
void O3_CPU::execute_store(uint32_t rob_index, uint32_t sq_index, uint32_t data_index)
{
    ...
    cout << " event_cycle: " << SQ.entry[sq_index].event_cycle << "\tcpu = " << cpu << endl; });
                ...
                for (uint32_t j=0; j<NUM_INSTR_SOURCES; j++) { // which one is dependent?
                    if (ROB.entry[dependent].source_memory[j] && ROB.entry[dependent].source_added[j]) {
                        if (ROB.entry[dependent].source_memory[j] == SQ.entry[sq_index].virtual_address) { // this is required since a single instruction can issue multiple loads
                            ...
                            cout << " full_addr: " << LQ.entry[lq_index].physical_address << dec << " is forwarded by store instr_id: ";
                            cout << SQ.entry[sq_index].instr_id << " remain_num_ops: " << ROB.entry[fwr_rob_index].num_mem_ops << " cycle: " << current_core_cycle[cpu] << "\tcpu = " << cpu << endl; });
...
void O3_CPU::complete_execution(uint32_t rob_index)
{
    if (ROB.entry[rob_index].is_memory == 0) {
        if ((ROB.entry[rob_index].executed == INFLIGHT) && (ROB.entry[rob_index].event_cycle <= current_core_cycle[cpu])) {
            ...
            cout << " event: " << ROB.entry[rob_index].event_cycle << "\tcpu = " << cpu << endl; });
        }
    }
    else {
        if (ROB.entry[rob_index].num_mem_ops == 0) {
            if ((ROB.entry[rob_index].executed == INFLIGHT) && (ROB.entry[rob_index].event_cycle <= current_core_cycle[cpu])) {
                ...
                cout << " fetch_stall: " << +fetch_stall << " event: " << ROB.entry[rob_index].event_cycle << " current: " << current_core_cycle[cpu] << "\tcpu = " << cpu << endl; });
            }
        }
    }
}
                ...
                if (ROB.entry[rob_index].registers_index_depend_on_me[i][j]) {
                    ...
                    if (ROB.entry[i].num_reg_dependent == 0) {
                        ...
                        if (ROB.entry[i].is_memory)
                            ...
                            cout << " head: " << RTE0_head << " tail: " << RTE0_tail << "\tcpu = " << cpu << endl; }); 
                    ...
                    cout << ROB.entry[i].instr_id << " reg_index: " << +ROB.entry[i].source_registers[j] << " num_reg_dependent: " << ROB.entry[i].num_reg_dependent << " cycle: " << current_core_cycle[cpu] << "\tcpu = " << cpu << endl; });
                }
            }
        }
    }
    ...
    if (queue->entry[index].instr_merged) {
        for (uint32_t i=0; i<ROB_SIZE; i++) {
            if (queue->entry[index].rob_index_depend_on_me[i]) {
                ...
                cout << " event_cycle: " << ROB.entry[i].event_cycle << "\tcpu = " << cpu << endl; });
            }
        }
    }
...
void O3_CPU::complete_data_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb)
{
    ...
    if (is_it_tlb) { // DTLB
        ...
        if (queue->entry[index].type == RFO) {
            ...
            cout << " load_merged: " << +queue->entry[index].load_merged << "\tcpu = " << cpu << endl; }); 
            ...
            cout << " head: " << RTL1_head << " tail: " << RTL1_tail << "\tcpu = " << cpu << endl; }); 
        ...
        if (queue->entry[index].type == RFO)
            ...
            cout << " load_merged: " << +queue->entry[index].load_merged << " inflight_mem: " << inflight_mem_executions << "\tcpu = " << cpu << endl; }); 
...
void O3_CPU::handle_o3_fetch(PACKET *current_packet, uint32_t cache_type)
{
    ...
    if (cache_type == 0) { // DTLB
        ...
        if (current_packet->type == RFO) {
            ...
            cout << " load_merged: " << +current_packet->load_merged << "\tcpu = " << cpu << endl; }); 
        ...
        if (current_packet->type == RFO)
            ...
            cout << " L1D_FETCH_DONE fetched: " << +LQ.entry[lq_index].fetched << hex << " address: " << (LQ.entry[lq_index].physical_address>>LOG2_BLOCK_SIZE);
            cout << " full_addr: " << LQ.entry[lq_index].physical_address << dec << " remain_mem_ops: " << ROB.entry[rob_index].num_mem_ops;
            cout << " load_merged: " << +current_packet->load_merged << " inflight_mem: " << inflight_mem_executions << "\tcpu = " << cpu << endl; }); 
    ...
    if (provider->store_merged) {
        for (uint32_t merged=0; merged<SQ.SIZE; merged++) {
            if (provider->sq_index_depend_on_me[merged]) {
                ...
                cout << " full_addr: " << SQ.entry[merged].physical_address << dec << " by instr_id: " << +provider->instr_id << "\tcpu = " << cpu << endl; });
            }
        }
    }
    if (provider->load_merged) {
        for (uint32_t merged=0; merged<LQ.SIZE; merged++) {
            if (provider->lq_index_depend_on_me[merged]) {
                ...
                cout << " full_addr: " << LQ.entry[merged].physical_address << dec << " by instr_id: " << +provider->instr_id << "\tcpu = " << cpu << endl; });
            }
        }
    }
}
...
void O3_CPU::handle_merged_load(PACKET *provider)
{
    for (uint32_t merged=0; merged<LQ.SIZE; merged++) {
        if (provider->lq_index_depend_on_me[merged]) {
            ...
            cout << " remain_mem_ops: " << ROB.entry[merged_rob_index].num_mem_ops << "\tcpu = " << cpu << endl; });
...
void O3_CPU::release_load_queue(uint32_t lq_index)
{
    ...
    cout << hex << " full_addr: " << LQ.entry[lq_index].physical_address << dec << "\tcpu = " << cpu << endl; });
...
void O3_CPU::retire_rob()
{
    for (uint32_t n=0; n<RETIRE_WIDTH; n++) {
        ...
        if (ROB.entry[ROB.head].executed != COMPLETED) { 
            DP ( if (warmup_complete[cpu]) {
            cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[ROB.head].instr_id << " head: " << ROB.head << " is not executed yet" << "\tcpu = " << cpu << endl; });
            return;
        }
        ...
        if (num_store) {
            if ((L1D.WQ.occupancy + num_store) <= L1D.WQ.SIZE) {
                ...
                DP ( if (warmup_complete[cpu]) {
                cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[ROB.head].instr_id << " L1D WQ is full" << "\tcpu = " << cpu << endl; });
        ...
        for (uint32_t i=0; i<MAX_INSTR_DESTINATIONS; i++) {
            if (ROB.entry[ROB.head].sq_index[i] != UINT32_MAX) {
                ...
                cout << " full_addr: " << SQ.entry[sq_index].physical_address << dec << "\tcpu = " << cpu << endl; });
        ...
        // release ROB entry
        DP ( if (warmup_complete[cpu]) {
        cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[ROB.head].instr_id << " is retired" << "\tcpu = " << cpu << endl; });
